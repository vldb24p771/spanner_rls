...
# 从文件中读取文本
input_file_path = "tem_result_rtt_waitdie.txt"  # 输入文件路径
...
for i in range(0, len(numbers_array1)):
    if i % 5 == 0:
        j+=1
    final_array1[i % 5] += numbers_array1[i]
    final_array2[i % 5] += numbers_array2[i]
...
# 存储数字到文件
output_file_path = "spanner_tput_rtt_waitdie.txt"  # 输出文件路径
